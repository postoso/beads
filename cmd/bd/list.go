...
func init() {
	...
	listCmd.Flags().String("format", "",
		"Dependency-graph export format: 'digraph' (edge list for "+
			"golang.org/x/tools/cmd/digraph), 'dot' (Graphviz), or a Go template. "+
			"All values except 'dot' are edge-scoped: one line per dependency "+
			"whose endpoints are both in the listing, so an edge-free listing "+
			"prints nothing and exits 0. Template fields are .IssueID, "+
			".DependsOnID, .Type, .Issue and .Dependency; there is no top-level .ID.")
